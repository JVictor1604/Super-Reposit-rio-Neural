...
def cria_candidato_caixeiro(cidades):
    ...
    pares = [nome for nome in nomes_cidades if int(nome.split()[-1]) % 2 == 0]

    # Embaralhar os dois grupos
    random.shuffle(impares)
    random.shuffle(pares)

    # Concatenar ímpares primeiro, depois pares
    caminho = impares + pares
    return caminho
...
def cruzamento_ordenado_com_preferencia(pai, mae, chance_de_cruzamento):
    ...
    pares_pai = [cidade for cidade in pai if int(cidade.split()[-1]) % 2 == 0]

    impares_mae = [cidade for cidade in mae if int(cidade.split()[-1]) % 2 == 1]
    pares_mae = [cidade for cidade in mae if int(cidade.split()[-1]) % 2 == 0]
    ...
    filho1 = filho1_impar + filho1_par
    filho2 = filho2_impar + filho2_par
...
def mutacao_troca_com_preferencia(populacao, chance_de_mutacao):
    ...
    for individuo in populacao:
        if random.random() < chance_de_mutacao:
            ...
            indices_pares = [i for i, cidade in enumerate(individuo) if int(cidade.split()[-1]) % 2 == 0]
